...
<head>
    ...
    <title>Acheter des Messages - GNTOMA</title>
...
<body class="min-h-screen pb-20">
    ...
    <header class="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-4 py-4">
        <div class="max-w-2xl mx-auto flex items-center justify-between">
            ...
            <h1 class="text-lg font-bold text-dark">Acheter des Messages</h1>
            <div class="w-10"></div>
        </div>
    </header>
    ...
    <main class="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div class="bg-red-50 border border-red-200 rounded-2xl p-4">
            <p class="text-sm font-bold text-red-700 text-center">Crédits insuffisants pour envoyer un message</p>
        </div>
        <?php endif; ?>

        <?php if ($success === 'purchased'): ?>
        <div class="bg-green-50 border border-green-200 rounded-2xl p-4">
            <p class="text-sm font-bold text-green-700 text-center">Achat réussi ! Vos crédits ont été ajoutés.</p>
        </div>
        <?php endif; ?>

        <!-- Solde actuel -->
        <div class="glass-panel rounded-[2rem] p-6 text-center">
            <p class="text-xs text-gray-500 font-bold uppercase mb-2">Votre solde actuel</p>
            <p class="text-5xl font-black text-dark mb-1"><?= number_format($credits['remaining_credits'] ?? 0) ?></p>
            <p class="text-sm text-gray-400">messages disponibles</p>
        ...
        <!-- Packs disponibles -->
        <div>
            <h2 class="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4">Packs disponibles</h2>
            ...
            <div class="space-y-4">
                ...
                <div class="pack-card glass-panel rounded-[2rem] p-6 cursor-pointer border-2 border-primary bg-primary/5" onclick="selectPack(1000, 2)">
                    ...
                    <p class="text-xs text-gray-500">$0.002 par message • Meilleur rapport qualité-prix</p>
                </div>
                ...
                <div class="pack-card glass-panel rounded-[2rem] p-6 cursor-pointer border-2 border-transparent hover:border-gray-200" onclick="selectPack(2500, 4)">
                    ...
                    <span class="inline-block mt-2 bg-orange-100 text-orange-700 text-[10px] font-bold px-2 py-1 rounded-full">Économisez 20%</span>
                </div>
            </div>
        </div>
        ...
        <form id="purchaseForm" action="messages_buy_process.php" method="POST" class="glass-panel rounded-[2rem] p-6">
            ...
            <div class="mb-5">
                <label class="flex items-center space-x-3 cursor-pointer">
                    ...
                    <span class="font-bold text-dark">C'est un cadeau</span>
                </label>
            </div>
            
            <!-- Destinataire du cadeau -->
            <div id="giftRecipient" class="mb-5 hidden">
                <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Code du destinataire</label>
                <input type="text" name="recipient_code" 
                       placeholder="Ex: A5, B12..."
                ...
                <p class="text-[10px] text-gray-400 mt-1">Le bénéficiaire recevra les crédits sur son compte</p>
            </div>

            <div class="flex items-center justify-between mb-4 p-4 bg-gray-50 rounded-xl">
                <div>
                    <p class="text-xs text-gray-500">Pack sélectionné</p>
                    <p class="font-bold text-dark"><span id="selectedSize">1000</span> messages</p>
                </div>
        ...
        <div class="glass-panel rounded-[2rem] p-5">
            <h2 class="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4">Historique d'achats</h2>
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    ...
                    <div class="text-right">
                        ...
                        <span class="text-[10px] bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">Cadeau</span>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endforeach; ?>
