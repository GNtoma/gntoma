...
<body class="min-h-screen pb-20">
    ...
    <main class="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div class="bg-green-50 border border-green-200 rounded-2xl p-4 animate__animated animate__bounceIn">
            <p class="text-sm font-bold text-green-700 text-center"><?= htmlspecialchars(__('journal_view.comment_sent'), ENT_QUOTES, 'UTF-8') ?></p>
        </div>
        <?php endif; ?>
        ...
        <div class="glass-panel rounded-[2rem] p-5 flex items-center space-x-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-xs text-gray-500"><?= htmlspecialchars(__('journal_view.author_label'), ENT_QUOTES, 'UTF-8') ?></p>
            </div>
        ...
        <div class="glass-panel rounded-[2rem] p-8 text-center">
            ...
            <h2 class="text-xl font-black text-dark mb-2"><?= htmlspecialchars(__('journal_view.paid_heading'), ENT_QUOTES, 'UTF-8') ?></h2>
            <p class="text-gray-500 text-sm mb-6"><?= htmlspecialchars(__('journal_view.paid_body'), ENT_QUOTES, 'UTF-8') ?></p>
            
            <?php if ($has_access_request): ?>
                <?php if ($access_request_status === 'pending'): ?>
                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                    <p class="text-sm font-bold text-yellow-700"><?= htmlspecialchars(__('journal_view.pending_notice'), ENT_QUOTES, 'UTF-8') ?></p>
                </div>
                <?php elseif ($access_request_status === 'rejected'): ?>
                <div class="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
                    <p class="text-sm font-bold text-red-700"><?= htmlspecialchars(__('journal_view.rejected_notice'), ENT_QUOTES, 'UTF-8') ?></p>
                </div>
                <a href="journal_access_request.php?journal_id=<?= $journal_id ?>" class="inline-block bg-primary text-white font-bold py-3 px-8 rounded-2xl hover:bg-blue-600 transition-all">
                    <?= htmlspecialchars(__('journal_view.renew_request'), ENT_QUOTES, 'UTF-8') ?>
                </a>
                <?php endif; ?>
            <?php else: ?>
            <a href="journal_access_request.php?journal_id=<?= $journal_id ?>" class="inline-block bg-primary text-white font-bold py-3 px-8 rounded-2xl hover:bg-blue-600 transition-all">
                <?= htmlspecialchars(__('journal_view.request_access'), ENT_QUOTES, 'UTF-8') ?>
            </a>
            <?php endif; ?>
        </div>
        <?php else: ?>
        ...
        <div class="space-y-4">
            ...
            <div class="glass-panel rounded-[2rem] p-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <p class="text-xs font-black uppercase tracking-widest text-gray-400">
                        <?= htmlspecialchars(__('journal_view.page_nav_label', ['current' => (string) $current_page, 'total' => (string) $total_pages]), ENT_QUOTES, 'UTF-8') ?>
                    </p>
                    <form method="GET" class="flex items-center gap-2">
                        <input type="hidden" name="id" value="<?= (int)$journal_id ?>">
                        <label for="goto-page" class="text-xs font-bold text-gray-500"><?= htmlspecialchars(__('journal_nav.go_to'), ENT_QUOTES, 'UTF-8') ?></label>
                        ...
                        <button type="submit" class="bg-primary text-white text-xs font-black px-4 py-2 rounded-xl hover:bg-blue-600 transition-all">
                            <?= htmlspecialchars(__('common.ok'), ENT_QUOTES, 'UTF-8') ?>
                        </button>
                    </form>
                </div>
            ...
            <div class="glass-panel rounded-[2rem] p-6 md:p-8">
                <div class="flex items-center space-x-2 mb-4">
                    ...
                    <h2 class="font-bold text-dark"><?= htmlspecialchars($page['title'] ?? __('journal_view.page_default_title', ['num' => (string) $current_page]), ENT_QUOTES, 'UTF-8') ?></h2>
                </div>
        ...
        <div class="glass-panel rounded-[2rem] p-6">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-black text-dark flex items-center space-x-2">
                    ...
                    <span><?= htmlspecialchars(__('journal_view.questions_title'), ENT_QUOTES, 'UTF-8') ?></span>
                </h3>
            ...
            <form action="journal_comment_add.php" method="POST" class="mb-6">
                ...
                <div class="relative">
                    <textarea name="content" rows="3" 
                              placeholder="<?= htmlspecialchars(__('journal_view.question_placeholder'), ENT_QUOTES, 'UTF-8') ?>" 
                ...
                <div class="flex justify-between items-center mt-3">
                    <p class="text-xs text-gray-400"><?= htmlspecialchars(__('journal_view.question_visibility'), ENT_QUOTES, 'UTF-8') ?></p>
                    <button type="submit" class="bg-primary text-white font-bold py-2 px-6 rounded-xl hover:bg-blue-600 transition-all text-sm flex items-center space-x-2">
                        ...
                        <span><?= htmlspecialchars(__('journal_view.send'), ENT_QUOTES, 'UTF-8') ?></span>
                    </button>
                </div>
            </form>
            <?php else: ?>
            <div class="bg-gray-50 border border-gray-200 rounded-2xl p-4 mb-6 text-center">
                <p class="text-sm text-gray-500"><?= htmlspecialchars(__('journal_view.login_to_ask'), ENT_QUOTES, 'UTF-8') ?></p>
                <a href="../index.php" class="inline-block mt-2 text-primary font-bold text-sm hover:underline"><?= htmlspecialchars(__('journal_view.sign_in'), ENT_QUOTES, 'UTF-8') ?></a>
            </div>
            <?php endif; ?>
            ...
            <div class="space-y-4" id="comments-list">
                ...
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-400 text-sm"><?= htmlspecialchars(__('journal_view.no_questions'), ENT_QUOTES, 'UTF-8') ?></p>
                    <p class="text-gray-300 text-xs mt-1"><?= htmlspecialchars(__('journal_view.be_first'), ENT_QUOTES, 'UTF-8') ?></p>
                </div>
                <?php else: ?>
                    ...
                    <div class="bg-gray-50 rounded-2xl p-4 relative group">
                                ...
                                <div class="flex items-start space-x-2">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center space-x-2 mb-1">
                                            <p class="font-black text-primary text-xs"><?= htmlspecialchars(__('journal_view.author_replied'), ENT_QUOTES, 'UTF-8') ?></p>
                        ...
                        <div class="mt-3 ml-12">
                            <button type="button" onclick="document.getElementById('reply-form-<?= $comment['id'] ?>').classList.toggle('hidden')" class="text-xs font-bold text-primary hover:underline flex items-center space-x-1">
                                ...
                                <span><?= htmlspecialchars(__('journal_view.reply_to_question'), ENT_QUOTES, 'UTF-8') ?></span>
                            </button>
                            ...
                            <form id="reply-form-<?= $comment['id'] ?>" action="journal_comment_add.php" method="POST" class="hidden mt-2 flex space-x-2">
                                ...
                                <input type="text" name="content" placeholder="<?= htmlspecialchars(__('journal_view.your_answer_placeholder'), ENT_QUOTES, 'UTF-8') ?>" class="flex-1 bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs focus:ring-1 focus:ring-primary outline-none" required>
